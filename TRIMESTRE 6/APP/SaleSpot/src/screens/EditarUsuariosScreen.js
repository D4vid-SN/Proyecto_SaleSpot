...
const EditarUsuariosScreen = ({ route }) => {
  ...
  const cargarRoles = async () => {
    try {
      ...
      Alert.alert('Error', 'No se pudo cargar la información de los roles');
    }
  };
  ...
  const handleActualizarUsuario = async () => {
    ...
    try {
      ...
      const response = await axios.put(`http://192.168.1.14:3000/usuarios/${tdoc_user}/${id_user}`, {
        ...
        email: email,
        rol: rol,
        estado: Boolean(estado),  // Forzar a booleano
      });

      setLoading(false);
      setExitoMensaje('Usuario actualizado exitosamente.');
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title="Actualizar Usuario" onPress={handleActualizarUsuario} disabled={loading} />
    </View>
  );
};
...
export default EditarUsuariosScreen;
